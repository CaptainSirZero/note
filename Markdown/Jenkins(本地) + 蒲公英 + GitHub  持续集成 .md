...
> 构建新版本时,之前的ipa如果想保留,可以设置ipa包名称按照时间戳
> 
> 本Git设置,Credentials相当于告诉GitLab插件,GitHub账号和密码,在自己的电脑上一般不用设置,需要设置的话,可以设置账号和密码,也可以上传密钥方式
> 
> 当前打包触发方式,需要自己设置,我这里没有设置,可以定时,或者安装githook
> 
> 构建环境中钥匙串和描述文件,一般在第一次设置时,会不显示我们之前上传的钥匙串和描述文件内容,保存退出,再次进入一次就可以了
> 
> Git上只有podfile文件,没有将pods文件上传上去,所以要执行一个脚本更新一下pod:`添加构建步骤 -> Execute shell`,需要放到最前,因为pods要在编译前就添加
